...
@test "Accept request with sharing PID namespace configuration set false" {
...
@test "Reject pod admission request with sharing PID namespace configuration set true" {
	run kwctl run  --request-path test_data/pod_with_sharing_pid_namespace_true_admission_request.json  annotated-policy.wasm
	[ "$status" -eq 0 ]
	[ $(expr "$output" : '.*"allowed":false.*') -ne 0 ]
 	[ $(expr "$output" : '.*"message":"pod cannot share host PID namespace".*') -ne 0 ]
}
